...
func bea_parkir(k *kendaraan) {
	/*  IS: k terdefinisi, kecuali durasi parkir (k.durasi) dan bea parkir (k.bea)
	    Proses: Memanggil prosedur durasi_parkir dan menghitung k.bea
		    Jika kendaraan mobil, biaya parkir jam pertama Rp 6000, per jam selanjutnya Rp 3000
			Jika kendaraan motor, biaya parkir jam pertama Rp 2500, per jam selanjutnya Rp 1500
	FS: Durasi parkir (k.durasi) berisi nilai dalam satuan jam
	*/
	durasi_parkir(k)
	if k.jke == "mobil" {
		k.bea = 6000 + 3000*(k.durasi-1)
	} else if k.jke == "motor" {
		k.bea = 2500 + 1500*(k.durasi-1)
	}
}

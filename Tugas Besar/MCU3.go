...
type tabPasien [NMAX]data_pasien
type tabLayanan [NMAX]paket_layanan

func main() {
	var dat_pas tabPasien
	var dat_lay tabLayanan
	dat_lay = [NMAX]paket_layanan{
        {kategori: "Umum", harga: 100000},
        {kategori: "BPJS", harga: 200000},
        {kategori: "Asuransi", harga: 300000},
        {kategori: "VIP", harga: 400000},
        {kategori: "VVIP", harga: 500000},
    }
	dat_pas = [NMAX]data_pasien{
        {nama: "shania", id: 12, waktu: periode{tahun: 2021, bulan: 9, tanggal: 13}, jenis: dat_lay[0], hasil_check_up: "panas"},
        {nama: "rahmania", id: 15, waktu: periode{tahun: 2022, bulan: 12, tanggal: 25}, jenis: dat_lay[1], hasil_check_up: "gamon"},
        {nama: "galuh", id: 17, waktu: periode{tahun: 2023, bulan: 3, tanggal: 9}, jenis: dat_lay[2], hasil_check_up: "HIV"},
        {nama: "ajeng", id: 11, waktu: periode{tahun: 2024, bulan: 10, tanggal: 3}, jenis: dat_lay[3], hasil_check_up: "sakit"},
        {nama: "soekarno", id: 10, waktu: periode{tahun: 2019, bulan: 1, tanggal: 31}, jenis: dat_lay[4], hasil_check_up: "kesurupan"},
        {nama: "soeharto", id: 13, waktu: periode{tahun: 2018, bulan: 8, tanggal: 17}, jenis: dat_lay[4], hasil_check_up: "cacar air"},
    }
	home(&dat_pas, &dat_lay)
}

func home(A *tabPasien, B *tabLayanan) {
	var opsi, opsi2 int
	var n, m int = 6, 5
	for opsi != 4 {
		fmt.Println("===================================================")
...
func edit_pasien(A *tabPasien, B *tabLayanan, n, m, idx int) {
	var nama, hasil_check_up string
	var opsi, tahun, bulan, tanggal int
	if idx == -1 {
		fmt.Println("Data Tidak Ditemukan")
	} else {
		...
		fmt.Scan(&hasil_check_up)
		if hasil_check_up != "-" {
			A[idx].hasil_check_up = hasil_check_up
		}
...
func cari_pasien_individu(A tabPasien, B tabLayanan, n int) {
	var opsi,y int
	var x string
	fmt.Println("Menu Cari Pasien")
...
func cari_pasien_periode(A tabPasien, B tabLayanan, n int) {
...
func display_pasien_periodik(A tabPasien,n, y1,m1,d1,y2,m2,d2 int)  {
	var i int
...
func cari_pasien_paket(A tabPasien, B tabLayanan, n,m int) {
	var opsi int
...
func edit_layanan(A *tabPasien, B *tabLayanan, n,m, idx int) {
	var nama string
	var harga int
...
func update_harga_layanan(A *tabPasien, B tabLayanan, n,idx int)  {
	for i := 0; i < n; i++ {
...
func main_hapus_paket(A *tabPasien, B *tabLayanan,n *int, m *int)  {
	var opsi int
...
func hapus_pasien_paket(A *tabPasien, B tabLayanan, n *int, idx int)  {
	for i := 0; i < *n; i++ {
		if A[i].jenis.kategori == B[idx].kategori{
...
func hapus_paket(B *tabLayanan, m *int, idx int)  {
	for i := idx; i < *m-1; i++ {
		B[i] = B[i+1]
	}
	*m--
...
func cek_nama_paket(B tabLayanan, m int, x string) bool  {
	var valid bool = true
	for i := 0; i < m; i++ {
		if B[i].kategori == x {
...
func main_display(A tabPasien, B tabLayanan, n int) {
	var opsi int
	var y1,m1,d1,y2,m2,d2 int
...
func cek_id_pasien(A tabPasien, n int, x int) bool {
	var i int = 0
	var uniq bool = true
	for i < n {
		if A[i].id == x{

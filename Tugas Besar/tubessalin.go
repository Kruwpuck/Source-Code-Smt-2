...
func cekBenar(BankSoal *TabSoal, answer FormatSoal, idx int) {

	if answer.jawaban == BankSoal.data[idx].jawaban {
		BankSoal.data[idx].correct++
		fmt.Println(" SELAMAT ANDA BENAR")
	} else {
...
func cekSalah(BankSoal *TabSoal, answer FormatSoal) {
	var i int
...
func Soal_Benar_Terbanyak(BankSoal *TabSoal) {
	...
	var i int
	maks = 0
	for i = 0; i < len(BankSoal.data); i++ {
		if BankSoal.data[i].correct > maks {
			maks = BankSoal.data[i].correct
...
func Quiz(BankSoal *TabSoal) {
	var i, j int
	var answer FormatSoal
	fmt.Println("*********************************************************************************************")
	fmt.Println("                                      SELAMAT MENGERJAKAN :)                                 ")
	fmt.Println("*********************************************************************************************")
	fmt.Println("Soal yang Akan anda kerjakan sebanyak:", BankSoal.n)
	...
	for i = 0; i < BankSoal.n; i++ {
		...
		fmt.Println("Masukan Jawaban:")
		fmt.Scan(&answer.jawaban)
		cekBenar(BankSoal, answer,answer.jawaban-1)

	}
}

func cekscore(BankSoal *TabSoal, User *TabUser) {

}

func sign_in(User *TabUser) {
	var peserta FormatUser
	...
	fmt.Scan(&peserta.FirstName)
	fmt.Println("Masukkan LastName: ")
	fmt.Scan(&peserta.LastName)
	fmt.Println("Masukkan Asal Kota: ")
	fmt.Scan(&peserta.AsalKota)
	fmt.Println("Masukkan Password: ")
	fmt.Scan(&peserta.password)
	fmt.Println(" Akun Anda Berhasil diBuat")
	fmt.Println("--------------------------------------------------------------------------------------------------------------")

	User.n++
	peserta.id = User.n
	User.data[User.n-1] = peserta
